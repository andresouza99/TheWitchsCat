        ...
        private int IdItem()
        {
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString)) //cria conexão com o bd
                {
                    ...
                    {
                        string query = "USE Projeto_G4 SELECT * FROM item_compra WHERE id = @id AND id_compra = @id_compra";
                        using (SqlCommand cmd = new SqlCommand(query, conn))
                        {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connectionString)) //cria conexão com o bd
                {
                    ...
                    if (achouacor)
                    {
                        string query2 = "USE Projeto_G4 SELECT id, qtd FROM item_compra WHERE id_compra = @id_compra AND id_produto = @id_produto AND id_cor = @id_cor";
                        using (SqlCommand cmd = new SqlCommand(query2, conn))
                        {
                            cmd.Parameters.AddWithValue("@id_compra", id);
                        ...
                        if (achouproduto)
                        {
                            ...
                            string queryupdate = "USE Projeto_G4 UPDATE item_compra SET qtd = @qtd, preco = @preco WHERE id = @id";
                        ...
                        {
                            string query4 = "USE Projeto_G4 INSERT INTO item_compra (id, id_compra, id_produto, id_cor, qtd, preco) VALUES (@id, @id_compra, @id_produto, @id_cor, @qtd, @preco)";
                        ...
                        FormCompra.compra.CarregarDados();
                        this.Close();
                    }
